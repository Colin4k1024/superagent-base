# Eino Dev Visual Orchestration Toolchain — Replacement Evaluation
...
Eino Dev is the visual orchestration + debugging toolchain shipped with
`cloudwego/eino`. It provides a canvas-style workflow editor, a node/edge
graph debugger, step-by-step tracing, and a dry-run surface tightly coupled
to the eino `compose` graph runtime.

The Go backend has completed its migration from eino to Google ADK Go as the
LLM framework (see `backend/pkg/agentdef/builder.go`, `adk_runner.go`,
`adk_stream.go`). With eino no longer the primary framework, Eino Dev loses
...
The project **already owns** the pieces Eino Dev used to provide externally:
...
| Runtime tracing hook | `backend/pkg/observe/eino_callback.go`, trace helpers | OpenTelemetry-style callbacks already emit per-node spans. |

**Conclusion:** the project does not lack a visual editor or a canvas format —
it lacks the **dev/debugging surface** Eino Dev layered on top (live trace
...
|---|---|---|
| **Keep Eino Dev** | Tightly coupled to eino `compose`; we left eino as the primary framework. Would require maintaining a now-orphaned eino dependency for tooling only. | ❌ Reject — drags a deprecated framework back in. |
...
cheapest, lowest-risk path is to fill the specific Eino Dev gap on top of assets
we already own.

### 4.1 Scope of the in-repo dev surface

1. **Live trace visualization.** The `eino_callback.go` / OpenTelemetry spans
   ...
   intermediate variables, and shows errors inline — mirroring Eino Dev's
   debug view without its eino coupling.
...
2. **Dry-run / single-node replay.** Add a `?dry_run=1` execution mode to
   ...
   persisting side effects. This replaces Eino Dev's step debugger.
...
4. **Canvas versioning.** Reuse the P2 "Agent version management" roadmap item
   ...
   editor undo/history parity with Eino Dev.
...
Replace Eino Dev with the in-repo dev surface described in §4. Do not adopt
