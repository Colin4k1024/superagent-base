...
| `hiagent_rag` | HiAgent 知识库检索 | query, user_id |

> `hiagent_rag` 需要配置环境变量才会激活，详见 [HiAgent RAG 集成](advanced/hiagent-rag)。
...
支持 per-tool 策略覆盖，详见 [Tool 沙盒模式](/advanced/sandbox)。
