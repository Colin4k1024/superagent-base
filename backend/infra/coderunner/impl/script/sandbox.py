...
class Sandbox:
    def __init__(
        ...
        # Ensure jsr.io and deno.land are always allowed for downloading JSR packages
        # This is required for @langchain/pyodide-sandbox to work
        # Deno needs these domains to download JSR package metadata and files
        required_jsr_hosts = ["jsr.io", "deno.land"]
        net_perms = [p for p in self.permissions if p.startswith("--allow-net")]
        if net_perms:
            # If --allow-net is specified, ensure required hosts are included
            net_perm = net_perms[0]
            if "=" in net_perm:
                # Extract existing allowed hosts
                allowed_hosts = net_perm.split("=", 1)[1]
                hosts_list = [h.strip() for h in allowed_hosts.split(",")]
                # Add required JSR hosts if not already present
                for host in required_jsr_hosts:
                    if host not in hosts_list:
                        hosts_list.append(host)
                # Replace the permission with updated hosts
                self.permissions = [p for p in self.permissions if not p.startswith("--allow-net")]
                self.permissions.append(f"--allow-net={','.join(hosts_list)}")
            # If --allow-net is set to True (no value), all hosts are already allowed
        else:
            # If no --allow-net is specified, add required JSR hosts explicitly
            self.permissions.append(f"--allow-net={','.join(required_jsr_hosts)}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        if params is not None:
                        # Use repr(params) to embed a safe Python literal, avoiding JSON quoting
            # issues (e.g. unescaped newlines) that can cause SyntaxError.
            # Safely embed params by passing JSON through a Python string literal,
            # then reconstructing the object with json.loads on the Pyodide side.
            # This avoids issues where newlines or quotes inside the data cause
            # unterminated string literal syntax errors when inlined directly.
            json_literal = repr(json.dumps(params, ensure_ascii=False))
            code = prefix + f"args = json.loads({json_literal})\n" + user_code + suffix
        else:
